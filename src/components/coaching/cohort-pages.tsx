...
import { assignments, cohorts, getCohort, getProfile } from "@/lib/demo-data";
...
export function CohortDetailPage({ id }: { id: string }) {
  const cohort = getCohort(id);

  if (!cohort) {
    return (
      <PageHeader
        description="La cohorte demandée n'existe pas dans les données démo."
        title="Cohorte introuvable"
      />
    );
  }

  const cohortAssignments = assignments.filter(
    (assignment) => assignment.assignedToCohortId === cohort.id,
  );
    ...
    <>
      ...
      <div className="space-y-6 p-6">
        <section className="grid gap-4 md:grid-cols-3">
          <StatCard
            helper={`${formatDate(cohort.startDate)} → ${formatDate(
              cohort.endDate,
            )}`}
            icon={UsersRound}
            label="Membres"
            value={String(cohort.memberIds.length)}
          />
          <StatCard
          ...
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
              {cohort.memberIds.map((memberId) => {
                ...
                return (
                  <div className="p-5" key={memberId}>
          ...
          <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="divide-y divide-slate-100">
              {cohortAssignments.map((assignment) => (
                <div className="grid gap-3 p-5 md:grid-cols-[1fr_120px]" key={assignment.id}>
                  <div>
