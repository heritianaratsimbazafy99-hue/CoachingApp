...
as $$
  select exists (
    ...
    left join public.cohort_members cm on cm.cohort_id = a.assigned_to_cohort_id
    where a.id = target_assignment_id
      and (
        a.assigned_to_user_id = target_user_id
        or cm.user_id = target_user_id
      )
  );
$$;
  ...
  or (
    ...
    and exists (
      ...
      where a.content_id = contents.id
        and public.user_can_see_assignment(a.id, auth.uid())
    )
  )
);
...
for select using (
  ...
  or public.user_can_see_assignment(id, auth.uid())
);
...
for insert with check (
  ...
  or (
    ...
    and (
      ...
      or public.coach_owns_coachee(auth.uid(), assigned_to_user_id)
    )
    and (
      assigned_to_cohort_id is null
      or exists (
        select 1 from public.cohorts c
        where c.id = assigned_to_cohort_id and c.coach_id = auth.uid()
      )
    )
  )
);
...
for select using (
  ...
  or exists (
    select 1 from public.cohort_members cm
    where cm.cohort_id = calendar_events.cohort_id
...
for all using (
  ...
  or exists (
    select 1 from public.cohort_members cm
    where cm.cohort_id = learning_paths.cohort_id
      and cm.user_id = auth.uid()
  )
)
with check (public.is_admin() or created_by = auth.uid());
  ...
  or exists (
    ...
    where lp.id = learning_path_items.learning_path_id
      and (
        ...
        or exists (
          select 1 from public.cohort_members cm
          where cm.cohort_id = lp.cohort_id and cm.user_id = auth.uid()
        )
      )
  )
)
